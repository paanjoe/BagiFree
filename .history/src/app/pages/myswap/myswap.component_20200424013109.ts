...
export class MyswapComponent implements OnInit {
  ...
  bar.then(() => {
    this.fileList = this.fileList.concat(testvar);
  });
  ...
  getBase64(file, orientation, fileName) {
    ...
    reader.onload = () => {
      ...
      this.imageCompress.compressFile(resetBase64Image, -2, 50, 50).then((res) => {
  ...
  beforeUploadSwap = (file: File): boolean => {
    const isJPG = file.type === 'image/jpeg';
    const isPNG = file.type === 'image/png';
    ...
    // Codename: EXIF
    this.getOrientationSwap(file, (orientation) => {
      this.getBase64Swap(file, orientation, fileName);
    });
  ...
  getBase64Swap(file, orientation, fileName) {
    ...
    reader.onload = () => {
      ...
      this.imageCompress.compressFile(resetBase64Image, -2, 50, 50).then((res) => {
       ...
       // this.dataURItoBlobSwap(resetBase64Image, fileName);
      });
    };
    reader.onerror = (error) => {
      console.log('Error: ', error);
  ...
  dataURItoBlobSwap(dataURI, fileName) {
    ...
    bar.then(() => {
      this.swapfileList = this.swapfileList.concat(this.test);
    });
  ...
  updateSwapRecord(data: Swap) {
    ...
    data.preferredItem = this.swapForm.controls['sItem'].value;
     ...
     this.productService.updateSwapProduct(data).subscribe((x) => {
       this.productService.insertSwapProductImageModal(data.id).subscribe(res => {
        this.swapfileList.forEach((itm) => {
          this.productService.insertSwapProductImage(0, data.id, itm.url).subscribe((response) => {
            this.isswapLoading = false;
            this.getSwapList(); 
          });
        });
