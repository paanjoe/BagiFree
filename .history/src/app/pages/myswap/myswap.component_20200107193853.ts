...
export class MyswapComponent implements OnInit {
  ...
  getProductAge(data: Products[]){
    data.forEach((element) => {
      var c = this.pAge.find((item: ProductsAge) => { return item.productAge_ID == element.productAge_ID});
  ...
  getProductCategory(data: Products[]) {
    data.forEach((element) => {
      var c = this.pCategory.find((item: ProductCategory) => { return item.categoryID == element.productCategory_ID});
  ...
  getProductCondition(data: Products[]) {
    data.forEach((element) => {
      var c = this.pCondition.find((item: ProductCondition) => { return item.productConditionID == element.productCondition_ID });
  ...
  handleSwapPreview = (file: UploadFile) => {
    ...
    console.log(this.swappreviewImage);
  }
