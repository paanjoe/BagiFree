...
export class HomeComponent implements OnInit {
  ...
  getswapProductCategory(data: Swap[]) {
    if (data !== [] && data !== null) {
      data.forEach((element) => {
        const c: ProductCategory = this.pCategory.find((item: ProductCategory) => item.categoryID === element.productCategory_ID);
        if (c !== null) {
  ...
  getswapProductCondition(data: Swap[]) {
    if (data !== []) {
      data.forEach((element) => {
        const c: ProductCondition = this.pCondition.find((item: ProductCondition) => item.productConditionID === element.productCondition_ID);
        if (c !== null) {
