...
export class HomeComponent implements OnInit {
  ...
  getProductAge(data: Products[]) {
    data.forEach((element) => {
      let c = this.pAge.find((item: ProductsAge) => item.productAge_ID == element.productAge_ID);
      element.age = c.productAgeTitle;
    });
  }
  ...
  getProductCategory(data: Products[]) {
    data.forEach((element) => {
      let c = this.pCategory.find((item: ProductCategory) => item.categoryID == element.productCategory_ID);
      element.category = c.categoryTitle;
    });
  }
  ...
  getProductCondition(data: Products[]) {
    data.forEach((element) => {
      let c = this.pCondition.find((item: ProductCondition) => item.productConditionID == element.productCondition_ID);
      element.condition = c.productConditionTitle;
    });
  }
